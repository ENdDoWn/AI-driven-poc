...
def ensure_nav_variant_change(components: list[ComponentNode], prompt: str) -> list[ComponentNode]:
    """Guarantee that a generic nav-change request produces a visible variant change."""
    prompt_lower = prompt.lower()
    if not any(keyword in prompt_lower for keyword in ("nav", "navbar", "เมนู", "นำทาง")):
        return components

    explicit_variant = next(
        (variant for variant in NAV_VARIANTS if variant in prompt_lower),
        None,
    )
    thai_variants = {
        "กลาง": "centered",
    ...
    explicit_variant = explicit_variant or next(
        (variant for keyword, variant in thai_variants.items() if keyword in prompt_lower),
        None,
    )

    def walk(nodes: list[ComponentNode]) -> bool:
        for node in nodes:
            if node.type == "navbar":
                current = node.props.get("variant", "minimal")
                if explicit_variant:
                    node.props["variant"] = explicit_variant
                else:
                    ordered = [
                        "minimal", "centered", "split", "announcement",
                        "glass", "underline", "sidebar",
                    ]
                    current_index = ordered.index(current) if current in ordered else 0
                    node.props["variant"] = ordered[(current_index + 1) % len(ordered)]
                if any(keyword in prompt_lower for keyword in ("cta", "button", "ปุ่ม", "ติดต่อ")):
                    node.props.setdefault("ctaText", "ติดต่อเรา")
                    node.props.setdefault("ctaHref", "#contact")
                return True
...
class LLMService:
    ...
    def __init__(self, api_key: str, model: str = "gemini-3-flash-preview"):
        ...
        """
        # Import Gemini only when an API-backed service is actually requested.
    ...
    async def generate_ui(
        ...
        try:
            ...
            if current_json is not None:
                ...
                parsed = ensure_nav_variant_change(parsed, prompt)
            else:
