...
from app.services.composer import compose_demo_page
...
async def generate_ui(request: GenerateRequest) -> GenerateResponse:
    ...
    if _llm_service is None:
        if request.current_json is None:
            return GenerateResponse(status="ready", components=compose_demo_page(request.prompt), prompt_used=request.prompt)
        validate_component_tree(request.current_json)
        return GenerateResponse(status="ready", components=request.current_json, prompt_used=request.prompt)
    ...
    try:
        ...
        return GenerateResponse(
            status="ready",
            components=validate_component_tree(components),
            prompt_used=request.prompt,
        )
